...
function btSave_postSubmit(button)
  {
	...
	FjmzhQueryJG_dataset.flushData(FjmzhQueryJG_dataset.pageIndex);
  }
...
function btBack_onClickCheck()
{
	...
	function btSaveEdit_onClickCheck(button){
		var jgOrganisationNameEN = FjmzhQueryJG_dataset.getValue("jgOrganisationNameEN");
	 	if(jgOrganisationNameEN == null || jgOrganisationNameEN == ""){
	    	alert("英文名称 不能为空！");
			return false;
	    }
	    var jgTIN = FjmzhQueryJG_dataset.getValue("jgTIN");
	    var jgIssuedBy = FjmzhQueryJG_dataset.getValue("jgIssuedBy");
	    var jgExplanation = FjmzhQueryJG_dataset.getValue("jgExplanation");
	 	if(jgTIN == null || jgTIN == ""){
